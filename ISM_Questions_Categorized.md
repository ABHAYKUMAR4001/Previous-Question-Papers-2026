...
### Q6 [2023-24 Mid-Sem Regular, Q1a - 3M]

Let A, B and C are three events such that $P(A) = 0.26$, $P(B) = 0.36$, $P(C) = 0.48$, $P(A \cap B) = 0.03$, $P(B \cap C) = 0.05$, $P(A \cap C) = 0.04$ and $P(A \cap B \cap C) = 0.01$.

Find: $P(A \cap B')$, $P(A' \cap B)$, $P(A' \cup B \cup C)$, $P(A \text{ only})$, $P(B \text{ only})$, $P(C \text{ only})$

### Q7 [2023-24 Mid-Sem Regular, Q1b - 4M]

Validate the statement "Mutually exclusive events are always independent." Validate with logic and examples in support.

### Q8 [2023-24 Mid-Sem Regular, Q2a - 3M]

Let A and B are two independent events such that $P(A) = 0.30$, $P(B) = 0.25$. Then find:
i) $P(B | A \cup B)$
ii) $P(A | A \cap B)$
iii) $P(A' | A \cup B)$
iv) $P(A' | A \cap B)$

### Q9 [2023-24 Mid-Sem Regular, Q2b - 4M]

In a manufacturing company, Machine 1 produces 40% of the items and Machine 2 and Machine 3 produce 25% and 35% of the items respectively. From past records, 15%, 20% and 25% of the items they produce are defective. Find:
i) Total percentage of defective produced
ii) If a defective item is selected randomly, find the probability that it is produced by Machine 1
iii) If a defective item is selected randomly, find the probability that it is NOT produced by Machine 2

### Q10 [2023-24 Mid-Sem Regular, Q3a - 4M]

Let $f(x) = k(x+1)/2$ where $0 < x < 2$, $0$ otherwise. Then find:
i) $k$ value
ii) $P(1/2 < x < 2)$
iii) $P(x > 1/4)$

### Q11 [2023-24 Mid-Sem Regular, Q3b - 4M]
...
i) Find marginal distribution of X
ii) Find marginal distribution of Y
iii) Find $P(X < 2 | Y < 2)$
iv) Find $P(X < 2 | Y = 1)$
...
### Q3 [2023-24 Mid-Sem Regular, Q4a - 4M]

Consider a random variable $X$ that follows binomial distribution with $n = 250$ and $p = 0.20$. Then find the probabilities (using normal approximation with continuity correction):
i) $P(X > 180)$
ii) $P(X = 150)$
iii) $P(150 < X < 180)$

### Q4 [2023-24 Mid-Sem Regular, Q4b - 4M]

Samples of size 45 are selected randomly from a population having mean 25 and standard deviation 4. Let $\bar{X}$ be a random variable associated with sampling distribution of means. Find the probability that:
i) $\bar{X}$ lies between 20 and 30
ii) $\bar{X}$ is greater than 28
iii) $\bar{X}$ is less than 28
